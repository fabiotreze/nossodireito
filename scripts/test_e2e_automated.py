...
class E2ETestRunner:
    ...
    def test_javascript_syntax(self) -> bool:
        ...
        critical_funcs = [
            'performSearch',              # Busca semântica (linha 854)
            'showDetalhe',                # Exibir detalhes de categoria (linha 667)
            'analyzeSelectedDocuments',   # Análise de documentos (linha 1451)
            'encryptBuffer',              # Criptografia AES-256-GCM
            'exportPdf',                  # Export análise para PDF (listener linha 1447)
            'renderCategories'            # Renderização de categorias (linha 643)
        ]
        ...
        # Pontuação: 100% = 6/6 funções
        if functions_found == len(critical_funcs):
            print(f"    ✅ Todas {functions_found}/{len(critical_funcs)} funções críticas presentes")
            return True
        elif functions_found >= 5:
            print(f"    ⚠️  {functions_found}/{len(critical_funcs)} funções críticas (falta 1)")
            return True
        elif functions_found >= 4:
            print(f"    ❌ {functions_found}/{len(critical_funcs)} funções críticas (faltam {len(critical_funcs) - functions_found})")
            return False
        else:
            print(f"    ❌ Apenas {functions_found}/{len(critical_funcs)} funções críticas")
            return False
