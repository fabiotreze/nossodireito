...
function resolveFile(urlPath) {
    ...
    const BLOCKED_DIRS = new Set(['terraform', 'codereview', 'node_modules', 'docs']);
...
- 100k+: Migrar para V2 com cache Redis + serverless (Azure Functions)
...
1. **Análise de PDFs (Acurácia ~70%)**
   ...
   - **Solução V2:** Azure OpenAI GPT-4o-mini (95% acurácia)
...
2. **Rate Limiting In-Memory (Não Distribuído)**
   ...
   - **Solução V2:** Redis (ioredis + rate-limiter-flexible)
...
6. **Não Suporta Laudos Físicos (Apenas Digital)**
   ...
   - **Solução V2:** OCR mobile (Tesseract.js ou Azure Computer Vision)
...
Ver [docs/roadmap/ROADMAP_V2.md](docs/roadmap/ROADMAP_V2.md) para arquitetura futura com:
